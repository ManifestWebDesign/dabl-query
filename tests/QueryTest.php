...
class QueryTest extends PHPUnit_Framework_TestCase {
	...
	function testGetQueryCount() {
		$q = new Query('test_table');
		$q->setAction(Query::ACTION_COUNT);
		$count_q = $q->getQuery($this->pdo)->__toString();
	...
	function testGroupBy() {
		...
		$q_string = $q->getQuery($this->pdo)->__toString();
	...
	function testOrderBy() {
		...
		$order_clause = $q->getQuery($this->pdo)->__toString();
...
ORDER BY `Title` DESC'), preg_replace('/\s/', '', $q->getQuery($this->pdo) . ''));
	}
	...
	function testAddJoinWithAlias() {
		...
		$this->assertEquals("SELECT `table`.*\nFROM `table`\n\tJOIN `testing` AS alias ON (alias.column = table.column)", (string) $q->getQuery($this->pdo));
	}
	...
	function testGetQueryTwoWordsWithAlias() {
		$q = new Query('SELECT testing', 'alias');
		$this->assertEquals("SELECT alias.*\nFROM SELECT testing AS alias", (string) $q->getQuery($this->pdo));
	}
	...
	function testJoinOnce() {
		...
		$this->assertEquals('JOIN `table2` ON (`table`.`column` = `table2`.`column`)', $join->getQueryStatement($this->pdo) . '');
	}
}